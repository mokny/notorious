...
# package-lock.json commonly picks up local diffs (platform-specific optional
# dependencies, npm version differences normalizing the file, ...) that would
# otherwise block `git pull` with "local changes would be overwritten by
# merge". It's regenerated by `npm install` a few lines down regardless, so
# any local edit to it is never worth keeping - discard before pulling.
