...
export const TEMPLATE_FILTERS: { name: string; detail: string }[] = [
  ...
  { name: "title", detail: "Title Case Each Word" },
  { name: "wordcount", detail: "Number of words" },
  { name: "replace", detail: "Replace all occurrences (replace(search, replacement))" },
  { name: "split", detail: "Split into a list (split(sep) - whitespace if omitted)" },
  { name: "slice", detail: "Substring/sublist (slice(start, end))" },
  { name: "contains", detail: "Does it contain a substring/item? (contains(needle))" },
  { name: "startswith", detail: "Starts with a prefix? (startswith(prefix))" },
  { name: "endswith", detail: "Ends with a suffix? (endswith(suffix))" },
  { name: "padstart", detail: "Pad on the left to a length (padstart(n, char))" },
  { name: "padend", detail: "Pad on the right to a length (padend(n, char))" },
  { name: "repeat", detail: "Repeat a string (repeat(n))" },
  { name: "regex", detail: "Does it match a pattern? (regex(pattern, flags))" },
  { name: "regexreplace", detail: "Replace by pattern (regexreplace(pattern, replacement, flags))" },
  { name: "regexextract", detail: "First match/capture group (regexextract(pattern, flags))" },
];
